...
class FloodWaitError(APIError):
    ...
    def __init__(self, seconds=None):
        message = f"Flood wait error. Wait for {seconds} seconds before next request" if seconds else "Flood wait error"
        super().__init__(message)
...
class ConfigFileNotFoundError(ConfigError):
    ...
    def __init__(self, file_path=None):
        message = f"Configuration file not found: {file_path}" if file_path else "Configuration file not found"
        super().__init__(message)
...
class ConfigValidationError(ConfigError):
    ...
    def __init__(self, issues=None):
        message = f"Configuration validation failed: {', '.join(issues)}" if issues else "Configuration validation failed"
        super().__init__(message)
