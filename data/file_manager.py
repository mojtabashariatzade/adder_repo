...
class FileManager:
    ...
    def move(self, src: Union[str, Path], dst: Union[str, Path],
        ...
        try:
            ...
            raise FileWriteError(str(dst_path), f"Error moving: {e}")
    ...
    def list_dir(self, path: Union[str, Path], pattern: Optional[str] = None) -> List[Path]:
        ...
        try:
            ...
            raise FileReadError(str(dir_path), f"Error listing directory: {e}")
...
class JsonFileManager(FileManager):
    ...
    def validate_json(self, path: Union[str, Path], schema: Dict[str, Any]) -> tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            logger.warning(f"JSON validation failed for {path}: {e.message}")
    ...
    def merge_json(self, target_path: Union[str, Path], source_path: Union[str, Path],
        ...
        logger.debug(f"Merged {source_path} into {target_path}")
        return target_data
...
class EncryptedFileManager(FileManager):
    ...
    def write_encrypted_json(self, path: Union[str, Path], data: Dict[str, Any],
        ...
        try:
            ...
            logger.debug(f"Encrypted JSON written to {path}")
        except TypeError as e:
            logger.error(f"Data is not JSON-serializable: {e}")
            raise TypeError(f"Data is not JSON-serializable: {e}")
        ...
        with lock:
            try:
                ...
                if make_backup and os.path.exists(path_str):
                    backup_path = f"{path_str}.bak.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                ...
                logger.debug(f"File written safely: {path_str}")
            except Exception as e:
...
def get_file_manager(manager_type: str = 'basic', *args, **kwargs) -> Union[FileManager, JsonFileManager, EncryptedFileManager]:
    """
    ...
    if manager_type.lower() == 'basic':
        ...
        logger.error(f"Unsupported file manager type: {manager_type}")
